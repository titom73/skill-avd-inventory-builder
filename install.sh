...
# Local installation script for generate_avd_data skill
# This script installs the skill from a local repository clone
#
# Usage:
#   Interactive mode:
#     ./install.sh
...
#     INSTALL_MODE=copilot ./install.sh
#     or with command line:
#     ./install.sh --copilot
#
#   Non-interactive mode for Claude Plugin:
#     INSTALL_MODE=claude ./install.sh
#     or with command line:
#     ./install.sh --claude
#
#   Skip PyAVD installation prompt:
#     INSTALL_MODE=copilot INSTALL_PYAVD=no ./install.sh
#     INSTALL_MODE=copilot INSTALL_PYAVD=yes ./install.sh
#
#   Skip Copilot instructions prompt:
#     INSTALL_MODE=copilot CREATE_COPILOT_INSTRUCTIONS=no ./install.sh
#     INSTALL_MODE=copilot CREATE_COPILOT_INSTRUCTIONS=yes ./install.sh
...
Examples:
  ...
  INSTALL_MODE=copilot INSTALL_PYAVD=yes CREATE_COPILOT_INSTRUCTIONS=yes ./install.sh
...
# Check if we're in the skill repository
if [[ ! -f "$SCRIPT_DIR/claude_plugin.json" ]]; then
    echo -e "${RED}Error: Not in the skill repository directory${NC}"
    echo "Please run this script from the skill repository root."
    exit 1
fi

# Check if we're in a git repo (for Copilot mode)
...
cp -f "$SCRIPT_DIR/claude_plugin.json" "$INSTALL_DIR/"
cp -f "$SCRIPT_DIR/README.md" "$INSTALL_DIR/"
cp -f "$SCRIPT_DIR/QUICK_INSTALL.md" "$INSTALL_DIR/"
cp -f "$SCRIPT_DIR/LICENSE" "$INSTALL_DIR/"

cp -f "$SCRIPT_DIR/docs/skills/generate_avd_data.md" "$INSTALL_DIR/docs/skills/"
cp -f "$SCRIPT_DIR/docs/skills/index.md" "$INSTALL_DIR/docs/skills/"
cp -f "$SCRIPT_DIR/docs/skills/metadata.json" "$INSTALL_DIR/docs/skills/"

cp -f "$SCRIPT_DIR/docs/examples/eos_designs_minimal_fabric.yml" "$INSTALL_DIR/docs/examples/"
cp -f "$SCRIPT_DIR/docs/examples/pyavd_hello_world.py" "$INSTALL_DIR/docs/examples/"
